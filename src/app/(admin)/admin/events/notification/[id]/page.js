...
const DocumentNotification = ({params}) => {
    ...
    const formik = useFormik({
	    ...
	    onSubmit: async (values) => {
			...
			f.append('id', event?.id);
            Action.sendEventNotification(f).then((data) => {
                if(data.success == true)
                {
                    console.log(data);
                    setLoader(false);
                ...
                toast.error('unable to send event notification');
                console.log(error);
                setLoader(false);
            });
		},
    ...
    return (
        <>
            <div className="border-b mb-8">
                <h1 className="text-3xl text-[#002855] font-light pb-5">Send Committee Meeting Notification</h1>
            </div>
            <div className="flex justify-start text-center">
                ...
                <Link href={`/admin/events/edit/${event?.id}`} className="w-44 py-2 px-6 bg-primary flex items-center justify-center font-medium ml-2">Edit</Link>
            </div>
            <div className="mt-6">
                <div className='flex'>
                    <strong>Title:</strong> <p className='ml-2'>{event?.title}</p>
                </div>
                <div className='mt-5 mb-5'>
                    <strong className='mb-2 block'>Description:</strong> 
                    <p dangerouslySetInnerHTML={{ __html:event?.discription}}></p>
                </div>
                <div className='flex'>
