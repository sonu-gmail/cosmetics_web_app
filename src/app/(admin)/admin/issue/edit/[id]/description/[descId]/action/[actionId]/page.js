...
const EditIssueDescriptionAction = ({params}) => {
    ...
    useEffect(() => {
        Action.getIssueDetail(params.id).then((data) => {
            if(data.success == true)
            {
                ...
                console.log(curentDescription);
                let descriptionActions = curentDescription.actions;
                let currentDescriptionAcion = descriptionActions.find(item => item.id == params.actionId)
                setIssueDescription(curentDescription);
                setEditorData(currentDescriptionAcion?.action);
                setExternalDeadline(currentDescriptionAcion?.external_deadline)
                setInternalDeadline(currentDescriptionAcion?.internal_deadline)
                setStatus(currentDescriptionAcion?.status_detail_id)
                setFieldValue("issue_type_id", params.id);
                setFieldValue("desc_id", params.descId);
                setFieldValue('external_deadline', currentDescriptionAcion?.external_deadline)
                setFieldValue('internal_deadline', currentDescriptionAcion?.internal_deadline)
                setFieldValue('status', currentDescriptionAcion?.status_detail_id)
            }
        }).catch((error) => {
    ...
    const formik = useFormik({
	    ...
	    onSubmit: async (values) => {
			...
			f.append('issue_desc_action_id', params.actionId);
            Action.updateIssueDescriptionAction(f).then((data) => {
                if(data.error == true) {
                    setLoader(false);
                ...
                toast.error('unable to update issue description action');
                console.log(error);
                setLoader(false);
            });
		},
    ...
    return (
        <>
            <div className="border-b mb-8">
                <h1 className="text-3xl text-[#002855] font-light pb-5">Edit Issue Decription Action</h1>
            </div>
                ...
                <div className="w-1/2 mr-8 border py-6 px-4">
                    <form onSubmit={handleSubmit} method="POST">
                        ...
                        <label className="block mb-4 mr-2">
                            ...
                            <select value={status} name="status" className="w-full h-[38px] py-2 px-2 outline-none bg-white border text-sm" onChange={handleChange}>
...
export default EditIssueDescriptionAction;